...
def model_inference(model, processor, messages, img_bytes_list, max_new_tokens):
    ...
    content = processor.decode(output_trimmed[0], skip_special_tokens=True)
...
def evaluate_test_image(model, processor, model_name, dataset_path, image_path, test_df, pdf_list, save_file_name):
    ...
    for i in tqdm(range(len(test_df))):
        ...
        for img_name in pdf_img_list:
            image_bytes = Image.open(f"{pdf_img_path}/{img_name}")
        ...
        full_content = []
        for img_path in pdf_img_list:
            img_dict = {
                "type": "image",
            }
            full_content.append(img_dict)

        text_dict = {
            "type": "text",
            "text": query,
        }
        full_content.append(text_dict)
        ...
        output = model_inference(model, processor, messages, img_bytes_list, max_new_tokens=8192)
...
def evaluate_test_text(model, processor, model_name, dataset_path, image_path, test_df, pdf_list, save_file_name):
    ...
    for i in tqdm(range(len(test_df))):
        ...
        output = model_inference(model, processor, messages, None, max_new_tokens=8192)
